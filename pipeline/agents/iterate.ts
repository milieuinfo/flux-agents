 ...
 * agents/shared/loop.ts), maximaal 3 rondes. Het verschil met `ship`: bij
 ...
 * Push en PR blijven bewuste manuele stappen (`npm run push`, `npm run pr`).
 *
 * Stopt verder bij ESCALATED (mens nodig) of na ronde 3.
 *
 * Usage:
 *   npm run iterate -- <TICKET-KEY> [sprintId]
  ...
  if (result.outcome === 'approved') {
    log.info(
      `\n✅ APPROVED na ronde ${result.round}. Lokale squash + _pr-body.md ` +
        `klaar. Niets gepusht. Push met 'npm run push -- ${key}${profileFlag}' ` +
        `en daarna 'npm run pr -- ${key}${profileFlag}'.`,
    );
  }
