...
void app.whenReady().then(async () => {
  // Gepackaged neemt macOS het dock-icoon uit het app-bundle (app/build/icon.icns).
  // In dev draait Electron kaal, dus zetten we het dock-icoon zelf zodat de
  // bliksem ook tijdens `npm run app:dev` zichtbaar is.
  if (!app.isPackaged && process.platform === 'darwin') {
    const img = nativeImage.createFromPath(join(repoRoot, 'app', 'build', 'icon.png'));
