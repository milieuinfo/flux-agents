...
import { runReview } from '../../pipeline/agents/review.js';
 ...
 * `npm run pipeline:review -- <KEY> --profile <naam>`. Rapporteert daarna de uitkomst
...
export async function reviewAction(): Promise<void> {
  ...
  if (isDesktop()) {
    ...
    p.log.success(wrapLog(`Gestart in een eigen tab: review ${key} (${profile}).`));
    return;
  }
