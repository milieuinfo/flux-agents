...
- Geïmplementeerd in `pipeline/jira/publish.ts` — directe REST-calls met `fetch`
...
4. `npm run jira:publish:dry -- <sprint>` om de gerenderde bodies in `_preview_*.md` lokaal te zien zonder Jira-calls
5. `npm run jira:publish -- <sprint> --tickets FLUX-XXX --skip-overview` om één test-comment te posten en de wiki-conversie in Jira te checken
6. Zet pas dan een volledige sprint live met `npm run jira:publish -- <sprint>`
