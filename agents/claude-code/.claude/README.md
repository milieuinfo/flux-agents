...
Deze map (`agents/claude-code/.claude/`) bevat de **interactieve Claude
...
De system prompts leven canoniek onder **`agents/prompts/`** (één bron van
waarheid). De SDK-agents (`agents/develop.ts`, `agents/review.ts`, …) laden
...
| mirror (`agents/`)    | canonical (`agents/prompts/`) |
|-----------------------|-------------------------------|
...
```
<!-- MIRROR — gesynced van agents/prompts/develop.md.
     Wijzig de canonical prompt (niet dit bestand) en herhaal de sync. -->
```

### Waarom een aparte kopie en niet "alles onder `agents/prompts/`"
...
1. **Claude Code dwingt de locatie af.** Een CC-subagent móét onder een
   ...
   `agents/prompts/` verhuizen zonder dat CC ze niet meer vindt.
...
`cat`-kopieën van iets onder `agents/prompts/`, en er is dan ook **geen
...
debuggen; de echte flow loopt via `npm run develop` / `npm run review`.
...
Bewerk de **canonical** prompt onder `agents/prompts/` en sync daarna de
mirrors:

```
npm run sync-cc-agents      # = scripts/sync-cc-agents.sh
```
